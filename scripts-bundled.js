...
(() => {
  ...
  var normalizeName = (name) => {
    if (!name.includes("-"))
      return name;
    if (["Necrozma-", "Calyrex-", "Kyurem-", "Ho-Oh"].some(
      (x) => name.startsWith(x)
    ))
      return name;
  ...
  var sentOutPokes = (player, pokes) => makePokemonSet(player.sentOut.map((x) => x.id)).isSuperset(
    makePokemonSet(pokes)
  );
  var getRestrictedPokes = (player) => {
    return player.team.filter(
      (x) => RestrictedPokemons.includes(normalizeName(x))
    );
  };
  ...
  var teamSentOut = (battles, onlyLeads = true, maxSentOutPairs = 3, getTeam = (p) => p.team) => {
    ...
    return [...data].map(([team, bs]) => {
      const data2 = categorize(
        bs,
        (b) => Immutable.Set(
          getOpponent(b).sentOut.slice(0, onlyLeads ? 2 : 4).map((x) => x.id)
        )
      );
